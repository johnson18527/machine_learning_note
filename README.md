...
#### 降低過擬風險
 - **更多訓練資料**：更多訓練資料能讓模型學到更有效的特徵，但直接增加實驗資料通常困難，但可以透過一定規則擴充資料，例如
圖分類的平移、旋轉、縮放，或生成對抗網路來合成大量新訓練資料

 - **降低模型複雜度**：資料較少但模型較複雜是過擬的主因，降低負責度可以防止模型擬合過多雜訊，如神經網路減少網路層、神經元
決策樹降低樹深度、進行剪枝

 - **正則化**：給模型參數加上一定的正則約束，例如將權值大小加到損失函數中，再最佳化目標函數同時，也能避免權值過高帶來的風險

 - **整合學習**：把多個模型整合在一起，降低單一模型的過擬風險,如bagging

#### 降低擬合不足風險
 - **增加新特徵**：當特徵不足或樣本相關性不夠，容易出現特徵不足，透過"挖掘上下文"、"id類特徵"、"組合特徵"等新特徵，往往能取
得更好的效果
 - **增加模型複雜度**：簡單模型的學習力較差，透過增加模型的複雜度可以增強擬合能力，如線性模型增高次向、神經網路增加網路層
數或神經元
 - **減少正則化系數**：正則化是防止過擬用的，但當模型擬合不足時則須減少正則化系數
...
### 決策樹有哪些啟發函數 
 ...
 - **ID3(最大資訊增益)**：計算各特徵資訊增益，資訊增益反映給定特定條件後，不確定性減少的程度，特徵取值越多也就等於定性更高，條件熵越小，資訊增益越大，實際應用中，這種分類的泛化能力非常弱，只能處理離散行變數，也只能處理分類問題，對缺失質敏感，透過剪枝來權衡樹的準確性與泛化能力
 - **C4.5(最大資訊增益比)**：計算各特徵資訊增益比，C4.5實際上是對ID3進行最佳化，透過引入資訊增益比，一定程度上對曲值比較多的特徵進行懲罰，避免ID3出現過擬，提身泛化能力，C4.5可以處理離散行變數及連續型變數，資料排序後找到類別不同的切割點將連續型轉成布林型也，每個特徵節點可以有多個分支，且特徵不重用，也只能處理分類問題，透過剪枝來權衡樹的準確性與泛化能力
 - **CART(最大吉尼係數)**：吉尼係數描述的是資料的純度，與資訊熵涵義類似，在每次迭代中選擇吉尼係數最小的特徵及其對應的切分點進行分類，且與ID3及C4.5不同，CART是二元樹，且特徵不重複，可以處理離散行變數及連續型變數，建構時會對特徵進行二值劃分可適用餘分類即回歸任務，回歸樹使用最小平方法則誤差準則，透過全部資料發現所有可能樹結構進行比對
